...
function renderTours(tours) {
    ...
    tours.forEach(function (tour) {
        ...
        const mobileBackdrop = tour.image
            ? `
                <div class="absolute inset-0 lg:hidden">
                    <img src="${escapeHtml(tour.image)}" alt="" class="h-full w-full object-cover" aria-hidden="true">
                    <div class="absolute inset-0 bg-gradient-to-r from-slate-950/85 via-slate-900/65 to-slate-900/25"></div>
                </div>
            `
            : `
                <div class="absolute inset-0 bg-gradient-to-br from-brand-700 via-brand-600 to-mint-600 lg:hidden"></div>
            `;

        const mobileTitleClass = tour.image ? "text-white" : "text-white";
        const mobileDetailClass = tour.image ? "text-slate-100" : "text-brand-50";

        const row = `

            <tr class="group border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors">

                <td class="hidden w-20 px-2 py-3 lg:table-cell lg:w-40 lg:px-5 lg:py-4">
                    ${tour.image
                ...
                <td class="relative overflow-hidden px-3 py-4 text-left align-middle lg:px-5 lg:py-4 lg:text-center">

                    ${mobileBackdrop}

                    <div class="relative z-10 mx-auto flex min-h-[92px] min-w-0 max-w-[280px] flex-col justify-center lg:block lg:min-h-0 lg:min-w-[220px]">

                        <div class="break-words font-semibold ${mobileTitleClass} lg:truncate lg:text-slate-900">

                            ${escapeHtml(tour.title)}

                        </div>

                        <div class="mt-1 flex items-center gap-2 text-xs font-medium ${mobileDetailClass} lg:hidden">
                            <span>${escapeHtml(tour.destination)}</span>
                            <span class="h-1 w-1 rounded-full bg-white/70"></span>
                            <span class="capitalize">${escapeHtml(tour.status)}</span>
                        </div>
                        ...
                        <div class="mt-1 line-clamp-2 text-sm ${mobileDetailClass} lg:truncate lg:text-slate-500">
                ...
                <td class="whitespace-nowrap bg-slate-50 px-2 py-3 text-center align-middle lg:bg-transparent lg:px-3 lg:py-4">

                    <div class="grid grid-cols-1 justify-center gap-1.5 lg:flex lg:flex-nowrap lg:gap-1">
                    <a  href="/tours/${tour.id}/"
                        class="inline-flex items-center justify-center rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-[11px] font-semibold text-slate-700 shadow-sm transition hover:border-brand-300 hover:bg-brand-50 hover:text-brand-700 lg:gap-1 lg:px-2.5 lg:py-2 lg:text-xs">
                    ...
                    <button
                        type="button"
                        class="editTourBtn inline-flex items-center justify-center rounded-lg border border-blue-100 bg-blue-50 px-2 py-1.5 text-[11px] font-semibold text-blue-700 shadow-sm transition hover:border-blue-200 hover:bg-blue-100 lg:gap-1 lg:px-2.5 lg:py-2 lg:text-xs"
                    ...
                    <button
                        type="button"
                        class="deleteTourBtn inline-flex items-center justify-center rounded-lg border border-red-100 bg-red-50 px-2 py-1.5 text-[11px] font-semibold text-red-700 shadow-sm transition hover:border-red-200 hover:bg-red-100 lg:gap-1 lg:px-2.5 lg:py-2 lg:text-xs"
                        data-tour-id="${tour.id}"
...
async function shareTour(title, tourId, joinCode) {
    window.location.href = `/tours/${tourId}/share/`;
}
